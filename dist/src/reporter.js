...
export function reportPaths(paths, options = {}) {
    if (!options.verbose) {
...
function reportVerbosePaths(paths) {
    ...
    console.log(chalk.dim(`summary: routes=${totalRoutes} paths=${paths.length} sinks=${sinkPaths} no-sink=${noSinkPaths} unresolved=${unresolvedPaths}`));
    ...
    for (const [routeIndex, [routeKey, routePaths]] of displayedRoutes.entries()) {
        ...
        for (const [pathIndex, tracedPath] of displayedPaths.entries()) {
            ...
            const allHops = [firstHop, ...remainingHops].filter((h) => Boolean(h));
            ...
            if (tracedPath.sink) {
                ...
                console.log(`  ${chalk.green("↳")} prisma.${tracedPath.sink.model}.${tracedPath.sink.operation}  ${relSinkFile}`);
            }
            else {
...
function reportDbAccessFiles(paths) {
    console.log("queryaware path analysis");
    console.log(divider);
    if (paths.length === 0) {
        console.log("  No routes found.");
...
export function reportIssues(issues) {
    ...
    for (const [file, fileIssues] of [...byFile.entries()].sort(([a], [b]) => a.localeCompare(b))) {
        ...
        for (const issue of fileIssues) {
            ...
            if (detail?.impact)
                console.log(chalk.dim(`     Impact: ${detail.impact}`));
            if (detail?.fix)
                console.log(chalk.dim(`     Fix:    ${detail.fix}`));
        }
        console.log("");
    }
    console.log(divider);
