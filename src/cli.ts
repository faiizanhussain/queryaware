...
export function runCli(argv: string[]): void {
  ...
  program
    .name("queryaware")
    .description(
      "queryaware — static analysis for database query anti-patterns.\n" +
  ...
  program
    .command("scan")
    .description(
      "Scan a folder for ORM query anti-patterns.\n" +
    ...
    .action((targetPath: string, options: { fix: boolean; skipGraph: boolean; skipStatic: boolean; verbose: boolean }) => {
  ...
  program
    .command("setup-husky")
    .description(
      "Add queryaware to your Husky pre-commit hook.\n" +
    ...
    .action((options: { path: string; hookFile: string }) => {
