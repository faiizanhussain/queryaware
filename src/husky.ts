...
function buildManagedBlock(targetPath: string): string {
  return `${START_MARKER}\nnpx @catisho/queryaware scan ${targetPath}\n${END_MARKER}`;
}

function upsertManagedBlock(content: string, targetPath: string): string {
...
export function setupHuskyPreCommit(targetPath: string, hookFilePath = ".husky/pre-commit"): void {
